...
class InteractionModel {
  ...
  def training_data(trnData:DataFrame,
    ...
    val consuming_trn = trnData
      .select("uid_flag","voi_complaint_count","voi_complrefund_count",
...
//    modelUtil.train_model(trndata_new_scal, sc, modelname = modelname)
  ...
  def modelling_data(tstData:DataFrame,
    ...
    val consuming_tst=tstData
      .select("voi_complaint_count","voi_complrefund_count","voi_comment_count",
        "acc_loginday_count","pro_validpoints","pro_base_active","pro_ctrip_profits",
        "pro_customervalue")
    ...
    val score_1        = predict_1_temp
      .withColumn("score_interaction",predict_1_temp("prediction_1")*500+350).select("score_interaction")
    //-------------------------------------------自此可以直接只返回RF----------------------------------------------------//
